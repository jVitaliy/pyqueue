...
class InterpreterTest(unittest.TestCase):
    ...
    def test_cd_and_rsync(self):
        ...
        self.assertEqual("cd /path/to && rsync -lr --files-from=<(find ./ -name \"*.git\") ./ /dest/folder", cmd_str)
    ...
    def test_cd_and_rsync_pattern_exclude(self):
        ...
        self.assertEqual("cd /path/to && rsync -lr --files-from=<(find ./ -name \".git\" -o -name \"*.yml\") "
                         "--exclude=*.desc --exclude=*.conf ./ /dest/folder", cmd_str)

    def test_cd_and_rsync_ssh_remote_dest(self):
        cmd_str = (RsyncCmdBuilder().cd("/path/to").rsync('lr').dest_ssh('git.tauproject.com', 'deploy', 'psw')
                   .dest_folder("/dest/folder").build())
        self.assertEqual("cd /path/to && rsync -lr ./ deploy:psw@git.tauproject.com/dest/folder", cmd_str)
